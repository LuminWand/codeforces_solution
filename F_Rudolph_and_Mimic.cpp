...
VVL a;
vector<map<LL, LL>> ct(6);

void gett(LL id, LL n) {
    LL i, j, k;
...
int main()
{
    ...
    while (t--) {
        ...
        // only keep the one with the increased freq
        LL cid = -1;
        for (auto p : ct[curr]) {
            if (p.second > ct[0][p.first]) cid = p.first;
        }

        VL r;       // remove
        FOR(i, n) {
            if (a[curr][i] != cid) r.push_back(i + 1);
        }
        cout << "- " << r.size() << " ";
        OUTV(r, r.size());
        cout.flush();
